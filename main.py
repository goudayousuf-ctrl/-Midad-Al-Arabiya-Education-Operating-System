...
# ------------------------------------------------------------------
# 2) لوحة القيادة: "اسأل مداد" — المحرك المركزي (Madad Intelligence Engine)
# ------------------------------------------------------------------
...
def ask_madad(req: AskMadadRequest):
    ...
    if "انخفض" in lower or "الصف الثالث" in lower:
        answer = (
            "📉 السبب المرجّح: غياب متكرر لثلاثة طلاب أثّر على المعدل العام، "
            "إضافة إلى تأخر تصحيح آخر واجبين.\n"
            "💡 الحل المقترح: جلسة تعويضية سريعة + تفعيل تنبيه غياب فوري لولي الأمر."
        )
    elif "أفضل" in lower or "معلم" in lower:
        answer = (
            "🏆 الترتيب: 1) أحمد محمد (96%) 2) سارة يوسف (94%) 3) فاطمة علي (88%) "
            "بناءً على الالتزام وسرعة التصحيح ورضا الطلاب."
        )
    elif "إيراد" in lower or "توقع" in lower:
        answer = (
            "💰 توقع الشهر القادم: نمو متوقع بنسبة 9% استنادًا لمعدل التجديد الحالي (91%) "
            "وانضمام 14 طالبًا جديدًا هذا الأسبوع."
        )
    elif "انقطاع" in lower or "تسرب" in lower:
        answer = (
            "⚠️ 3.1% من الطلاب ضمن منطقة الخطر — أغلبهم بسبب تأخر الرد على واجبين متتاليين. "
            "يُنصح بتدخل مبكر خلال 48 ساعة."
        )
    else:
        answer = (
            f'📊 تحليل عام لسؤالك ("{q}"): المؤشرات ضمن النطاق الصحي '
            "(حضور 96.4%، رضا أولياء أمور 4.7/5). اسأل عن معلم أو طالب أو منهج محدد لتحليل أدق."
        )
...
def generate_lesson(req: LessonRequest):
    plan = (
        ...
        "• أهداف الدرس: إتقان المفردات والتراكيب المستهدفة في الموضوع المطلوب.\n"
...
# ------------------------------------------------------------------
# 6) التقارير المتقدمة: تحليل تنفيذي شامل
# ------------------------------------------------------------------
@app.post("/api/executive-analytics")
def executive_analytics():
    report = (
        "📈 التقرير التشخيصي الشامل:\n"
        "• الأداء الأكاديمي العام ضمن النطاق الممتاز (فوق 90% في 5 من 6 مؤشرات رئيسية).\n"
        "• أعلى فرصة لرفع الكفاءة: تقليص زمن تصحيح الواجبات لدى 20% من المعلمين.\n"
        "• التوقع: استمرار النمو بمعدل 8-10% شهريًا إن استمر معدل التجديد الحالي."
    )
    return {"status": "success", "executive_report": report}


# ------------------------------------------------------------------
# 7) الإدارة والمالية: مستشار التطوير المالي والإداري
# ------------------------------------------------------------------
@app.post("/api/financial-advisor")
def financial_advisor():
    report = (
        "💰 التقرير المالي والتطويري الإداري:\n"
        "• الإيرادات التشغيلية: ممتازة، مع نمو ملحوظ في الاشتراكات بنسبة 15%.\n"
        "• أجور المعلمين: محسوبة آلياً بدقة بناءً على حصص الشهر.\n"
        "• توصية للتطوير: تفعيل حزمة مكافآت تحفيزية للمعلمين الحاصلين على تقييم ممتاز في المرور الإشرافي."
    )
